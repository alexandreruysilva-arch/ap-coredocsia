...
// Endpoint OpenAI-compatível da Moonshot / Kimi (platform.kimi.ai)
const KIMI_URL = "https://api.moonshot.ai/v1/chat/completions";

/**
 * Extrai valores de indexação da(s) primeira(s) página(s) de um documento
 * (imagem) via API da Kimi (Moonshot) e grava log de auditoria em
 * `ai_usage_logs`. Segue o mesmo contrato das demais IAs.
 */
export const extractFieldsWithKimi = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!apiKey) throw new Error("KIMI_API_KEY não configurado no servidor");
    ...
    if (!isImage) {
      await writeFailureLog({
        ...
        completion: 0,
        total: 0,
        error: `Tipo de arquivo não suportado pelo Kimi: ${mimeType}`,
      });
      throw new Error(
        "Kimi aceita apenas imagens (JPG, PNG, GIF, WEBP). Converta antes de enviar.",
      );
    }
...
- Se não encontrar com confiança, retorne "".`;
    ...
    if (!resp || !resp.ok) {
      await writeFailureLog({
        ...
        completion: 0,
        total: 0,
        error: `Kimi ${lastStatus || "network"}: ${lastErrText.slice(0, 200)}`,
      });
      const friendly =
        lastStatus === 401
          ? "KIMI_API_KEY inválida. Verifique a chave da Kimi (Moonshot)."
          : lastStatus === 429
            ? "Limite de requisições atingido (Kimi). Aguarde alguns segundos."
            : lastStatus >= 500
              ? "O serviço Kimi está temporariamente indisponível. Tente novamente."
              : `Falha ao processar via Kimi (${lastStatus || "rede"}): ${lastErrText.slice(0, 120)}`;
