...
/**
 * Extrai valores de indexação da(s) primeira(s) página(s) de um documento (imagem/PDF)
 * via API da OpenAI (Chat Completions com vision) e grava log de auditoria em `ai_usage_logs`.
 */
export const extractFieldsWithOpenAI = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!apiKey) throw new Error("OPENAI_API_KEY não configurado no servidor");
    ...
    if (!isImage) {
      await writeFailureLog({
        ...
        completion: 0,
        total: 0,
        error: `Tipo de arquivo não suportado pelo OpenAI: ${mimeType}`,
      });
      throw new Error(
        "OpenAI aceita apenas imagens (JPG, PNG, GIF, WEBP). Converta antes de enviar.",
      );
    }
...
- Se não encontrar com confiança, retorne "".`;
    ...
    if (!resp || !resp.ok) {
      await writeFailureLog({
        ...
        completion: 0,
        total: 0,
        error: `OpenAI ${lastStatus || "network"}: ${lastErrText.slice(0, 200)}`,
      });
      const friendly =
        lastStatus === 401
          ? "OPENAI_API_KEY inválida. Verifique a chave da OpenAI."
          : lastStatus === 429
            ? "Limite de requisições atingido (OpenAI). Aguarde alguns segundos."
            : lastStatus >= 500
              ? "O serviço OpenAI está temporariamente indisponível. Tente novamente."
              : `Falha ao processar via OpenAI (${lastStatus || "rede"}): ${lastErrText.slice(0, 120)}`;
